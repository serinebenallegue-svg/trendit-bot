...
📌 بعد الدفع، أرسلي صورة وصل الدفع هنا لتأكيد الطلبية.
"""
...
def cart_buttons(cart):
    ...
    for i in range(len(cart)):
        buttons.append([InlineKeyboardButton(f"🗑️ حذف المنتج {i+1}", callback_data=f"remove_{i}")])
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ تم ربط هذا الحساب كأدمن. ستصلك الطلبيات هنا.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "مرحبا بك في Trend Eight 🛍️\n\n"
        "أرسلي رابط منتج Shein للبدء."
    )
    ...
    if user_data[user_id].get("step") == "waiting_price":
        try:
            ...
            if len(cart) >= 3:
                ...
                await update.message.reply_text("أرسلي رابط منتج آخر أو اكتبي /cart لعرض السلة.")

        except:
            await update.message.reply_text("أرسلي السعر بشكل صحيح. مثال: 1.78 أو 1,78")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data[user_id].get("step") != "waiting_receipt":
        await update.message.reply_text("تم استلام الصورة، لكن لا توجد طلبية قيد التأكيد.")
        return
...
async def show_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cart = user_data[user_id]["cart"]
    await update.message.reply_text(cart_text(cart), reply_markup=cart_buttons(cart))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("remove_"):
        ...
        if len(cart) == 0:
            ...
            await query.message.reply_text(cart_text(cart), reply_markup=cart_buttons(cart))
